...
def saveExperience(path, exp_name, py_list, sh_list):
    ...
    new_exp_name = str(next_exp_nb) + exp_name
...
def saveAllExperience(path,exp_name):
    next_exp_nb = experienceNumber(path)
    new_exp_name = str(next_exp_nb) + '_' + exp_name
    os.makedirs(os.path.join(path,new_exp_name))
    os.makedirs(os.path.join(path,new_exp_name,'Sets'))
    os.makedirs(os.path.join(path,new_exp_name,'Sets','Test'))
    os.makedirs(os.path.join(path,new_exp_name,'Sets','Training'))
    os.makedirs(os.path.join(path,new_exp_name,'Results'))
    return new_exp_name
...
def saveAsJson(dict):
    ...
    with open(dict["result_path_exp"]+dict["subexp_name"] + '/'+'dictionnary.json', 'w') as json_file:
