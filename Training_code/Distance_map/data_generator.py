...
from image_augmentation_3_channels import *
...
class DataGenerator(keras.utils.Sequence):
    ...
    def prepare_batch(self, X, Y):
        ...
        Y_augm = np.zeros(Y.shape, dtype=float)
        for i in range(X.shape[0]):
            if Y is not None:
                X_augm[i], Y_augm[i] = self.imaugment(X[i], Y[i])
            else:
                X_augm[i], a = self.imaugment(X[i], None)
    ...
    def imaugment(self, X, Y):
        ...
        if self.params["augmentation"]["augmentation_choices"][0] == True:
            ## Y = [y1 , y2]
            X, Y = randomTransform(X, Y, self.params['augmentation']["random_transform"], self.params['augmentation']['save_folder'])
        return X, Y
